<template>
  <div class="containerBig">
    <br />
    <h1>Formulario para crear Ejercicios de ajedrez</h1>
    <h3>Autor del ejercicio: {{ nouElement.autor }}</h3>
    <br />
    <div class="container mt-3">
      <form v-if="!ejercicioCreadoBool && !ejercicioErrorServidor" @submit.prevent="afegirElement">
        ...
        <div class="form-group radio-group">
          ...
          <div
            class="radio-options"
            :class="{ 'is-invalid': procesando && dificultadInvalido }"
          >
            <div>
              <input
                type="radio"
        ...
        <div class="form-group">
          ...
          <a href="https://lichess.org/editor" target="_blank" title="le conduce al editor de posiciones de lichess ">obten el FEN</a>
        ...
        <div class="form-group">
          ...
          <div
            class="radio-options"
            :class="{ 'is-invalid': procesando && colorInvalido }"
          >
            <div>
              <input
                type="radio"
        ...
        <div class="form-group">
          <label for="texto"
            >Texto explicativo del ejercicio (opcional) :</label
          >
          <textarea
        ...
        <button type="submit" class="btn btn-success" title="añadir el ejercicio a la base de datos ">Añadir Ejercicio</button>
      </form>
      ...
      <div class="alert alert-success" role="alert" v-if="ejercicioCreadoBool">
        <h4>Ejercicio creado correctamente</h4>
        <button @click="nuevoEjercicio" class="btn btn-success">
          Añadir otro Ejercicio
        </button>
      </div>
      <div class="alert alert-danger" role="alert" v-if="ejercicioErrorServidor">
        <h4>Disculpas, ha habido un error insertando el ejercicio, pruebe otra vez</h4>
        <button @click="nuevoEjercicio" class="btn btn-success">
          Añadir Ejercicio
        </button>
      </div>
...
export default {
  ...
  methods: {
    ...
    checkPgnValidity(cadena) {
      ...
      const regex =
        /^(?:[a-zA-Z]{1,2}\d|\d{1,2}\.|(?:\d{1,2})\.{3}|0-0|0-0-0|[a-zA-Z]{2}\d[a-zA-Z]|[a-zA-Z]\d[a-zA-Z])$/;

      // /^(?:[a-zA-Z]{1,2}\d|\d{1,2}\.|(?:\d{1,2})\.{3}|0-0|0-0-0)$/;
    ...
    nuevoEjercicio() {
      this.ejercicioCreadoBool = false;
      this.ejercicioErrorServidor = false;
    },
    afegirElement() {
      ...
      this.nouElement.id = Math.floor(Math.random() * 1000000).toString;
      this.postEjercicio(this.nouElement);

      this.error = false;
      this.correcto = true;
      this.procesando = false;
      // També pots reiniciar les dades del formulari si és necessari
      this.nouElement.dificultad = ""; 
      this.nouElement.tema = "";
      this.nouElement.fen = "";
      this.nouElement.solucion = "";
      this.nouElement.texto = "";
      this.nouElement.color = ""; 
      this.nouElement.enunciado = "";
    ...
    async postEjercicio(ejercicio) {// para añadir nuevo ejercicio a la base
      try {
  ...
  computed: {
    ...
    textoInvalido() {
      return this.nouElement.texto.length < 0; // provisional no es obligado
    },
    enunciadoInvalido() {
  ...
  mounted() {
    ...
    this.nouElement.autor = localStorage.getItem("savedUser") || "";// autor sera el usuario
  },
};
</script>
